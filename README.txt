...
d. If the build succeeds, and you are satisfied with the test results, run
 ...
 e. If you want to create a tarball for subsequent installations:
    ...
    $ cd /
    $ sudo tar -zxvf <build_dir>/gcc-builder/packages/kewb-gcc5X0*.tgz

    or, alternatively:

    $ sudo tar -zxvf ./gcc-builder/packages/kewb-gcc5X0*.tgz -C /
    ...
    $ cd /usr/local
    $ sudo chown -R root:root gcc/5.X.0/
    $ sudo chown root:root bin/*gcc5X0*
...
of the following two actions:
  ...
  b. Rename the custom 'as' and 'ld' exectuables, for example:
    ...
    $ cd /usr/local/gcc/5.X.0/libexec/gcc/x86_64-kewb-linux-gnu/5.X.0
    $ mv as as-old
    $ mv ld ld-old
