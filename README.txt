================================================================================
 ...
 KEWB Computing
================================================================================
This is the README file for the KEWB GCC 10.X.0 build scripts.  In the following
text, the version numbers will be referred to as 10.X.0 or 10X0, depending on the
usage and context.
...
--------------------------------------------
4. HOW TO BUILD GCC 10.X.0 WITH THESE SCRIPTS
...
The process is pretty simple:
 ...
 b. Customize the variables exported by gcc-build-vars.sh.  In particular,
    ...
    GCC_VERSION, to select the version of GCC 10.X.0 to download and build.
...
d. If the build succeeds, and you are satisfied with the test results, run
 ...
 e. If you want to create a tarball for subsequent installations:
    ...
    $ cd /
    $ sudo tar -zxvf <build_dir>/gcc-builder/packages/kewb-gcc10X0*.tgz

    or, alternatively:

    $ sudo tar -zxvf ./gcc-builder/packages/kewb-gcc10X0*.tgz -C /
    ...
    $ cd /usr/local
    $ sudo chown -R root:root gcc/10.X.0/
    $ sudo chown root:root bin/*gcc10X0*
...
If you want to rebuild GCC 10.X.0 after having built and installed it according
...
is imperative that you first perform one of the following three actions:
  ...
  a. Delete the installation directory, for example:
    ...
    $ rm -rf /usr/local/gcc/10.X.0

  --OR--

  b. Rename the installation directory, for example:

    $ mv /usr/local/gcc/10.X.0 /usr/local/gcc/10.X-old

  --OR--

  c. Rename the custom 'as' and 'ld' exectuables, for example:

    $ cd /usr/local/gcc/10.X.0/libexec/gcc/x86_64-kewb-linux-gnu/10.X.0
    $ mv as as-old
    $ mv ld ld-old

Otherwise, the configure portion of the build process will find the custom
'as' and 'ld' executables in the GCC 10.X.0 directory structure, and build
...
installed in the GCC 10.X.0 directory structure.


---------------------------------------------
5. HOW TO USE THE KEWB CUSTOM GCC 10.X.0 BUILD

Before using the compiler, some paths need to be set.  The simplest way to do
this is source the "setenv-for-gcc10X0.sh" script that is installed.

 a. Source the script /usr/local/bin/setenv-for-gcc-10X0.sh, which was installed
    in step 4.e or 4.f above.  For example,

        $ source /usr/local/bin/setenv-for-gcc10X0.sh
...
-- OR --
 ...
 a. You will need to modify your PATH environment variable so that the
    ...
    /usr/local/bin).  For example,
        ...
        $ export PATH=/usr/local/gcc/10.X.0/bin:$PATH
 ...
 b. You will also need to modify your LD_LIBRARY_PATH environment variable so
    ...
    appear first in the path.  For example,
        ...
        $ export LD_LIBRARY_PATH=/usr/local/gcc/10.X.0/lib:\
          /usr/local/gcc/10.X.0/lib64:$LD_LIBRARY_PATH
